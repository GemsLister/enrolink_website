...
export default function HeadLogin() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (res.role !== 'DEPT_HEAD') {
        setError('Head account required.')
        return
      }
  ...
  return (
    <AuthLayout title="Department Head Login" illustration={illo} logo={logo} greetings="Good to see you, Department Head">
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <button disabled={loading || !captcha} className="w-full bg-[#8a1d35] text-white rounded-md py-2 disabled:opacity-50">{loading? 'Logging in...' : 'Log in as Head'}</button>
