...
export function RecordsPanel({ token, view = 'applicants', basePath }) {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      const regularResp = await fetch('https://fonts.gstatic.com/s/arimo/v20/P5sYzZCDf9_T_32nY7u6Hw.ttf')
      const boldResp = await fetch('https://fonts.gstatic.com/s/arimo/v20/P5sQzZCDf9_T_32nG4m8Lw.ttf')
      if (regularResp.ok) {
        const fontBlob = await regularResp.blob()
      ...
      if (boldResp.ok) {
        const fontBlobB = await boldResp.blob()
        const fontBase64B = await new Promise((resolve) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result).split(',')[1]); reader.readAsDataURL(fontBlobB); })
        doc.addFileToVFS('Arimo-Bold.ttf', fontBase64B)
        doc.addFont('Arimo-Bold.ttf', 'Arimo', 'bold')
      }
      doc.setFont('Arimo', 'normal')
    } catch (e) { try { doc.setFont('helvetica', 'normal') } catch(_) {} }
      ...
      const mb = 14
    const titleY = 24
    const infoYStart = titleY + 6
    const headerLines = 9
    const lineGap = 5
    const contentTop = infoYStart + headerLines * lineGap + 4
    ...
    autoTable(doc, {
      ...
      styles: { font: 'Arimo', fontSize: 7, cellPadding: 1, overflow: 'linebreak', cellWidth: 'wrap' },
      headStyles: { font: 'Arimo', fillColor: [220, 220, 220], textColor: [0, 0, 0] },
      ...
      didDrawPage: (dataCtx) => {
          ...
          if (logoData) {
            ...
            doc.setFontSize(12)
            doc.text('EnroLink', pw / 2, 14, { align: 'center' })
          }
          doc.setFontSize(16)
          doc.text(pdfTitle, pw / 2, titleY, { align: 'center' })
          const uni = ['Bukidnon State University','Malaybalay City, Bukidnon','Contact no.: 09067247572','enrolink@gmail.com']
          const summaryTitle = isStudentView ? 'STUDENT SUMMARY' : (isEnrolleeView ? 'ENROLLEE SUMMARY' : 'APPLICANT SUMMARY')
          const actorName = String(user?.name || '').trim()
          const roleLabel = (user?.role === 'DEPT_HEAD' ? 'Department Head' : (user?.role === 'OFFICER' ? 'Enrollment Officer' : String(user?.role || '')))
          const now = new Date()
          const dateGenerated = now.toLocaleDateString()
          const timeGenerated = now.toLocaleTimeString()
          const startYear = now.getFullYear()
          const academicYear = `S.Y. ${startYear}-${startYear + 1}`
          const semesterLabel = '—'
          let y = infoYStart
          doc.setFontSize(10)
          for (const line of uni) { doc.text(line, pw / 2, y, { align: 'center' }); y += lineGap }
          doc.setFontSize(12)
          try { doc.setFont('Arimo', 'bold') } catch(_) {}
          doc.text(summaryTitle, pw / 2, y, { align: 'center' })
          try { doc.setFont('Arimo', 'normal') } catch(_) {}
          y += lineGap + 1
          doc.setFontSize(10)
          doc.text(`Generated by: ${actorName || '—'}`, ml, y)
          y += lineGap
          doc.text(`Role: ${roleLabel || '—'}`, ml, y)
        y += lineGap
        doc.text(`Date Generated: ${dateGenerated}`, ml, y)
        doc.text(`Time Generated: ${timeGenerated}`, pw - mr, y, { align: 'right' })
        y += lineGap
        doc.text(`Academic Year: ${academicYear}`, ml, y)
        doc.text(`Semester: ${semesterLabel}`, pw - mr, y, { align: 'right' })
